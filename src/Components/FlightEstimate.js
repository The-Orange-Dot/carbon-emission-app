  ...
  const [results, setResults] = useState({
    ...
    function handleFormSubmit(formData) {
        fetch("https://www.carboninterface.com/api/v1/estimates", {
            ...
            "legs": [
                {"departure_airport": formData.origin, "destination_airport": formData.destination},
            ],
            "distance_unit": "mi"
            })
        })
  ...
  return (
    <div>
      <div className="homepage-container">
        <h1 className="welcome-text">Calculate carbon emissions for your flights</h1>
      </div>
      <FlightForm handleFormSubmit={handleFormSubmit}/>
      {results.id.length !== 0 ? <FlightResults flightData={results} onSaveDataClick={onSaveDataClick}/> : null}
    </div>
  );
}
